...
export function TransactionsTable({ transactions, accounts }: TransactionsTableProps) {
  ...
  const { uploadingTxId, handleReceiptUpload } = useReceiptUpload();
  ...
  function handleAutoSplit(tx: Transaction, virtualAccountId: number) {
    ...
    setEditingAllocations({
      ...editingAllocations,
      [tx.id]: buildAutoSplitAllocations(selectedAcc, virtualAccounts, tx.amount, tx.type),
    });
  }
  ...
  function handleAddLine(tx: Transaction) {
    ...
    setEditingAllocations({
      ...editingAllocations,
      [tx.id]: addAllocationRow(current, defaultAcc.id, tx.amount, tx.type),
    });
  }
  ...
  function handleRemoveLine(txId: number, index: number) {
    ...
    setEditingAllocations({
      ...editingAllocations,
      [txId]: removeAllocationRow(current, index),
    });
  }
  ...
  function handleUpdateLine(
    ...
    setEditingAllocations({
      ...editingAllocations,
      [txId]: updateAllocationRow(current, index, field, value),
    });
  }
              ...
              return (
                <tbody key={tx.id} style={{ borderBottom: "2px solid var(--border-color)" }}>
                  <tr>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35rem" }}>
                        {tx.virtualAllocations.length === 0 ? (
                          ...
                          tx.virtualAllocations.map((v, i) => {
                            ...
                            return (
                              <Badge key={i} variant="virtual" style={{ fontSize: "0.75rem" }}>
                    ...
                    <td>
                      <MatchingStatusBadge status={status} difference={difference} />
                    </td>
